...
class MapTask extends Task {
		...
		if (supportIsolationRunner(conf) && isMapOrReduce()) {
			...
			Path localSplitMeta = 
					new LocalDirAllocator("mapred.local.dir").getLocalPathForWrite(
							TaskTracker.getLocalSplitFile(conf.getUser(), getJobID().toString(), getTaskID().toString()), conf);
  ...
  private <INKEY,INVALUE,OUTKEY,OUTVALUE> void runOldMapper(final JobConf job, final TaskSplitIndex splitIndex, 
		  final TaskUmbilicalProtocol umbilical, TaskReporter reporter) throws IOException, InterruptedException, ClassNotFoundException {
  ...
  private <INKEY,INVALUE,OUTKEY,OUTVALUE> void runNewMapper(final JobConf job, final TaskSplitIndex splitIndex, 
	  ...
	  org.apache.hadoop.mapreduce.RecordReader<INKEY,INVALUE> input =
			  new NewTrackingRecordReader<INKEY,INVALUE> (split, inputFormat, reporter, job, taskContext);
  ...
  class MapOutputBuffer<K extends Object, V extends Object> implements MapOutputCollector<K, V>, IndexedSortable {
	  ...
	  @SuppressWarnings("unchecked")
	  public MapOutputBuffer(TaskUmbilicalProtocol umbilical, JobConf job, TaskReporter reporter) throws IOException, ClassNotFoundException {
		  this.job = job;
		  this.reporter = reporter;
	  ...
	  public synchronized void collect(K key, V value, int partition) throws IOException {
		  ...
		  try {
			  ...
			  mapOutputByteCounter.increment(valend >= keystart
					  ? valend - keystart : (bufvoid - keystart) + valend);
					  ...
					  if (kvstart == kvend) {
						  ...
						  if (kvend != kvindex) {
							  ...
							  final boolean bufsoftlimit = (bufindex > bufend)
									  ? bufindex - bufend > softBufferLimit : bufend - bufindex < bufvoid - softBufferLimit;
  ...
  /*
   * java.io.IOException: hdfs://paradent-13.rennes.grid5000.fr:9000/user/kkloudas/grep-temp-1570502682/part-r-00000 not a SequenceFile
	...
	at org.apache.hadoop.mapred.MapTask$TrackedRecordReader.<init>(MapTask.java:193)
