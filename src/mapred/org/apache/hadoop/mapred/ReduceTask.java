...
public class ReduceTask extends Task {
	...
	@Override
	public TaskRunner createRunner(TaskTracker tracker, TaskInProgress tip, TaskTracker.RunningJob rjob) throws IOException {
	...
	public void run(JobConf job, final TaskUmbilicalProtocol umbilical)
		...
		final TaskReporter reporter = new TaskReporter(getProgress(), umbilical, jvmContext);
		...
		//VISCO: if the job is not local then create the merging tree and do the reduce		
		boolean isLocal = "local".equals(job.get("mapred.job.tracker", "local"));
		if (!isLocal) {
			reduceCopier = new ReduceCopier(umbilical, job, reporter);		    

			List<MapOutputLocation> locs = reduceCopier.getMapOutputLocations();
		
			copyPhase.complete();                         // copy is already complete
			setPhase(TaskStatus.Phase.SORT);			  // set sort as the next phase
			...
			MergingTree.createMergingTree(job, finalOutputName, locs, getJobTokenSecret(), threadPool,
				...
				public void action() {
					try {
						...
						// Kostas : this is a new method. 
						// It was the JVM bug : we were killing the JVM and then trying to fetch map events.
						reduceCopier.closeMapEventsThread();
	...
	public class OldTrackingRecordWriter<K, V> implements RecordWriter<K, V> {
		...
		public OldTrackingRecordWriter(org.apache.hadoop.mapred.Counters.Counter outputRecordCounter,
				JobConf job, Reporter reporter, String finalName) throws IOException {
	...
	private <INKEY,INVALUE,OUTKEY,OUTVALUE> void runOldReducer(JobConf job,
		...
		final RecordWriter<OUTKEY, OUTVALUE> out = new OldTrackingRecordWriter<OUTKEY, OUTVALUE>(reduceOutputCounter, job, reporter, finalName);
	...
	public class NewTrackingRecordWriter<K,V> extends org.apache.hadoop.mapreduce.RecordWriter<K,V> {
		...
		public NewTrackingRecordWriter(org.apache.hadoop.mapreduce.Counter recordCounter, JobConf job, 
				Reporter reporter, org.apache.hadoop.mapreduce.TaskAttemptContext taskContext) throws InterruptedException, IOException {
			
			this.outputRecordCounter = recordCounter;
			this.fileOutputByteCounter = reporter.getCounter(org.apache.hadoop.mapreduce.lib.output.FileOutputFormat.Counter.BYTES_WRITTEN);
			Statistics matchedStats = null;
			...
			long bytesOutPrev = getOutputBytes(fsStats);
			this.real = (org.apache.hadoop.mapreduce.RecordWriter<K, V>) outputFormat.getRecordWriter(taskContext);
			long bytesOutCurr = getOutputBytes(fsStats);
			fileOutputByteCounter.increment(bytesOutCurr - bytesOutPrev);
		}
	...
	public class ReduceCopier<K, V> implements MRConstants {
		...
		private class MapOutputCopier extends Thread {
			...
			private MapOutput getMapOutput(MapOutputLocation mapOutputLoc, Path filename, int reduce) throws IOException, InterruptedException {
				...
				//VISCO: changed to false to force shuffling on disk only
				boolean shuffleInMemory = false; //ramManager.canFitInMemory(decompressedLength); 
		...
		public ReduceCopier(TaskUmbilicalProtocol umbilical, JobConf conf, 
			...
			this.maxFetchFailuresBeforeReporting = conf.getInt("mapreduce.reduce.shuffle.maxfetchfailures", REPORT_FAILURE_LIMIT);
		...
		/**
		 * 
		 * @return the List of MapOutputLocations
		 * @throws IOException
		 */
		public List<MapOutputLocation> getMapOutputLocations() throws IOException {

			//start getting the hosts
			...
			//loop until we get all the required URLs
			
			// Kostas : here I also had the condition for terminanting but it is not needed. 
			while (scheduledCopies.size() < numMaps) {
				...
				synchronized (scheduledCopies) {
					...
					while (hostsItr.hasNext()) {
						...
						// is cleared once the jobtracker restarts and is rebuilt
						// from scratch.
						// Note that map-output-location mapping will be recreated 
						// and hence we continue with the hope that we might find
						// some locations from the rebuild map and add then for fetching.
						...
						synchronized (knownOutputsByLoc) {
							...
							while (locItr.hasNext()) {
								...
								//we have to be sure that the returned List doesn't have duplicates
								if(!(scheduledCopies.contains(loc))) {
			...
			return scheduledCopies;
		}

		public void closeMapEventsThread() {
			// Kostas : shouldn't I use a join or something for synchronization ?
			...
			public void run() {
				...
				do {
					try {
						...
						if (LOG.isDebugEnabled()) {
							if (numNewMaps > 0) {
								LOG.debug(reduceTask.getTaskID() +": Got "+ numNewMaps +" new map-outputs."); 
							}
						}
						Thread.sleep(SLEEP_TIME);
					} catch (InterruptedException e) {
						LOG.warn(reduceTask.getTaskID() +" GetMapEventsThread returning after an interrupted exception");
						return;
					} catch (Throwable t) {
						String msg = reduceTask.getTaskID() +" GetMapEventsThread Ignoring exception : "
								+ StringUtils.stringifyException(t);
						reportFatalError(getTaskID(), t, msg);
					}
